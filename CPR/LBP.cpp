#include "stdafx.h"
#include "LBP.h"
#define BORDER_DEFAULT cv::BORDER_REFLECT_101
//获取i中0,1的跳变次数
...
void LBP::ComputeLBPFeatureVector_256(const cv::Mat &srcImage, cv::Size cellSize, cv::Mat &featureVector)
{
	// 参数检查，内存分配
...
void LBP::ComputeLBPFeatureVector_Uniform(const cv::Mat &srcImage, cv::Size cellSize, cv::Mat &featureVector)
{
	// 参数检查，内存分配
	CV_Assert(srcImage.depth() == CV_8U && srcImage.channels() == 1);
...
// cellSize:每个cell的大小,如16*16
void LBP::ComputeLBPFeatureVector_Rotation_Uniform(const cv::Mat &srcImage, cv::Size cellSize, cv::Mat &featureVector)
{
	// 参数检查，内存分配
	CV_Assert(srcImage.depth() == CV_8U && srcImage.channels() == 1);
...
void LBP::ComputeLBPImage_Rotation_Uniform(const cv::Mat &srcImage, cv::Mat &LBPImage)
{
	// 参数检查，内存分配
	CV_Assert(srcImage.depth() == CV_8U && srcImage.channels() == 1);
	LBPImage.create(srcImage.size(), srcImage.type());
...
void LBP::ComputeLBPImage_Rotation_Uniform_2(const cv::Mat &srcImage, cv::Mat &LBPImage)
{
	// 参数检查，内存分配
	CV_Assert(srcImage.depth() == CV_8U && srcImage.channels() == 1);
	LBPImage.create(srcImage.size(), srcImage.type());
...
//	qDebug() << "旋转不变一共有" << numberOfMinBinary << "种";
	...
	{
		//std::cout << "旋转不变的LBP：" << number[i] << " " << "对应的等价模式：" << table[number[i]] << endl;
	}
